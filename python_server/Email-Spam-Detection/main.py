...
model : RandomForestClassifier =pickle.load(open('Email-Spam-Detection-model.pkl','rb'))
...
def check_for_spam2(input_email):
    transform_txt=transform_text(input_email)
    print(f">Transformed text : {transform_txt}\n")

    vector_input=tfidf.transform([transform_txt])   #vectorization
    print(f">Vectorized text : {vector_input}\n")
